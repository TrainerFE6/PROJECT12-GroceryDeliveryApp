  ...
  const onSubmit: SubmitHandler<FormLogin> = async (data) => {
    try {
      const handleLogin = async (data: FormLogin) => {
        setIsLoading(true)
        const res = await instance.post(`/auth${data.type === 'customers' ? '-customers' : ''}/login`, data)
        setIsLoading(false)
        ...
        if (res?.tokens?.access?.token) {
          ...
          navigate(`/${data.type === 'customers' ? '' : 'backoffice'}`);
        }
      }
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-4">
                  ...
                  <select
                    ...
                    required
                    {...register("type")}
                    className="w-full py-4 pl-6 bg-white border border-stroke rounded-md text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  >
                ...
                <div className="mb-5">
                  ...
                  <button
                    disabled={isLoading}
                    type="submit"
                    className={`w-full cursor-pointer rounded-lg border border-primary bg-primary ${isLoading ? '!border-slate-500 !bg-gray !text-black' : ''} p-4 text-white transition hover:bg-opacity-90`}
                  >
                    Masuk Sekarang
                  </button>
                </div>
